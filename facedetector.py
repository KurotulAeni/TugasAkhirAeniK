...
class FaceDetector:
    ...
    def _transformToData(integral_image, wndW, wndH, padX, padY):
        ...
        for y in range(0, h-wndH, padY):
            for x in range(0, w-wndW, padX):
                data.append(
                    [integral_image[y:y+wndH+1, x:x+wndW+1], x, y, wndW, wndH])
    ...
    def detect(self, image, min_size=0.0, max_size=1.0, step=0.5, detectPad=(12, 12), verbose=False, getTotalTiles=False):
        ...
        si = max_size
        while True:
            scaledimg = sp.misc.imresize(image, size=(
                int(si*height), int(si*width)), mode='F')
            integral_image = self.boostedCascade.translateToIntegralImage(
                scaledimg)
